    ...
    public:
        ...
        void ploter()
        {
            while (ros::ok())
            {
                ...
                if (leg_points_steps.point1.size() > 0)
                {
                ...
                if (leg_points_steps.point2.size() > 0)
                {
                ...
                if (leg_points_steps.point3.size() > 0)
                {
                ...
                if (leg_points_steps.point_next.size() > 0)
                {
                    cube_colors.assign(leg_points_steps.point_next.size(),yellow_color);
                    pub_marker_leg_next.publish( marker_lib.makeMarkerList( visualization_msgs::Marker::SPHERE_LIST, all_parameter.robot_base, "point_next", defalt_pose, leg_points_steps.point_next, cylinder_scale, cube_colors, ros::Duration(1) ) );
                }
